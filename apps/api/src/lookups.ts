...
export function installLookupRoutes(app: FastifyInstance, database: Database, authenticate: AuthenticateRequest): void {
  ...
  app.get<{ Reply: OrganizationalUnitsResponse }>('/lookups/organizational-units', { preHandler, schema: { response: { 200: OrganizationalUnitsResponseSchema, ...errors } } }, async (request, reply) => {
    try { const rows = await findActiveOrganizationalUnits(database, { institutionId: request.principal.institutionId, authorizationContext: request.principal.authorization }); return reply.code(200).send({ items: rows.map((row) => ({ id: row.id, code: row.code, name: row.name })) }); }
  ...
  app.get<{ Reply: AccessClassificationsResponse }>('/lookups/access-classifications', { preHandler, schema: { response: { 200: AccessClassificationsResponseSchema, ...errors } } }, async (request, reply) => {
    try { const rows = await findAccessClassifications(database, { institutionId: request.principal.institutionId, authorizationContext: request.principal.authorization }); return reply.code(200).send({ items: rows.map((row) => ({ id: row.id, legalClassification: row.legal_classification, operationalVisibility: row.operational_visibility })) }); }
    catch (error) { throw mapLookupError(error); }
  });
}
