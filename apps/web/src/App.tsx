...
import { apiRequest, can, fetchSession, type Capability, type HealthResponse, type Session } from './api.js';
import { EmptyState, ErrorState, ForbiddenState, LifecycleBadge, LoadingState, NotFoundState, OfflineState } from './ux.js';

interface NavigationItem { readonly label: string; readonly path: string; readonly icon: typeof HomeOutlinedIcon; readonly capability?: Capability; }
interface NavigationGroup { readonly label: string; readonly items: readonly NavigationItem[]; }

const primaryNavigation: readonly NavigationGroup[] = [{ label: 'Trabajo', items: [{ label: 'Inicio', path: '/work', icon: HomeOutlinedIcon }, { label: 'Asuntos', path: '/matters', icon: AssignmentOutlinedIcon, capability: 'records.read' }, { label: 'Expedientes', path: '/expedientes', icon: FolderOutlinedIcon, capability: 'records.read' }, { label: 'Archivo', path: '/archive', icon: ArchiveOutlinedIcon, capability: 'records.read' }] }];
...
export function App(): ReactElement {
  ...
  const health = useQuery({ queryKey: ['system-health'], queryFn: () => apiRequest<HealthResponse>('/health'), retry: false });
...
function AppShell({ session, sessionLoading, sessionError, health, healthLoading, healthError }: { readonly session: Session | null | undefined; readonly sessionLoading: boolean; readonly sessionError: Error | null; readonly health: HealthResponse | undefined; readonly healthLoading: boolean; readonly healthError: Error | null }): ReactElement {
  ...
  const adminItems = administrationNavigation.filter((item) => session !== null && session !== undefined && can(session, item.capability!));
...
function NavigationDrawer({ session, currentPath, adminItems, onNavigate }: { readonly session: Session | null | undefined; readonly currentPath: string; readonly adminItems: readonly NavigationItem[]; readonly onNavigate: () => void }): ReactElement {
  return <Box sx={{ px: 1.5, py: 2 }}><Typography color="text.secondary" sx={{ px: 1.5, mb: 1 }} variant="overline">Módulos</Typography>{primaryNavigation.map((group) => <List aria-label={group.label} key={group.label} sx={{ mb: 1 }}>{group.items.filter((item) => item.capability === undefined || (session !== null && session !== undefined && can(session, item.capability))).map((item) => <NavigationLink item={item} currentPath={currentPath} key={item.path} onNavigate={onNavigate} />)}</List>)}{adminItems.length > 0 ? <><Divider sx={{ my: 1 }} /><Typography color="text.secondary" sx={{ px: 1.5, mb: 1 }} variant="overline">Administración</Typography><List aria-label="Administración">{adminItems.map((item) => <NavigationLink item={item} currentPath={currentPath} key={item.path} onNavigate={onNavigate} />)}</List></> : null}<Box sx={{ px: 1.5, pt: 3 }}><Chip label={session === null ? 'Modo público' : 'Contexto institucional'} size="small" variant="outlined" /></Box></Box>;
}
...
export function MattersRoute(): ReactElement { const context = useShellContext(); return <ProtectedPage capability="records.read" session={context.session} sessionLoading={context.sessionLoading}><SectionPage description="Bandejas y seguimiento operativo de asuntos." title="Asuntos" /></ProtectedPage>; }
...
export function ProtectedPage({ capability, children, session, sessionLoading }: { readonly capability: Capability; readonly children: ReactElement; readonly session: Session | null | undefined; readonly sessionLoading: boolean }): ReactElement { if (sessionLoading) return <LoadingState />; if (session === null || !can(session, capability)) return <ForbiddenState />; return children; }
