...
import { integrationQueueName, processIntegrationJob, runMalwareScanOnce, type MalwareScanWorkerDependencies } from './jobs.js';
...
const malwareDependencies: MalwareScanWorkerDependencies | undefined = config.databaseUrl !== undefined && config.clamavHost !== undefined && config.s3Endpoint !== undefined && config.s3AccessKeyId !== undefined && config.s3SecretAccessKey !== undefined && config.s3QuarantineBucket !== undefined && config.s3CleanBucket !== undefined
  ...
  : undefined;
let polling = false;
const pollTimer = malwareDependencies === undefined ? undefined : setInterval(() => {
  if (polling) return;
  polling = true;
  void runMalwareScanOnce(malwareDependencies).catch((error: unknown) => console.error(JSON.stringify({ event: 'malware.poll_failed', message: error instanceof Error ? error.message : 'poll failed' }))).finally(() => { polling = false; });
