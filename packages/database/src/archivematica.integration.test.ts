...
describe('Archivematica durable references', () => {
  ...
  beforeAll(async () => {
    ...
    await database.insertInto('expedientes').values({ id: expedienteId, institution_id: institutionId, folio: 'EXP-2026-000001', folio_year: 2026, sequence_number: 1, status: 'CLOSED', expediente_type_version_id: versionId, metadata: {}, opened_at: new Date() }).execute();
    await database.insertInto('archive_transfers').values({ id: transferId, institution_id: institutionId, expediente_id: expedienteId, status: 'APPROVED' }).execute();
  }, 120_000);
