...
export class TenantRepositories {
  ...
  public async markJobAttempt(id: string): Promise<void> { this.requireAffected(await this.transaction.updateTable('integration_jobs').set(({ eb }) => ({ status: 'RUNNING', attempt_count: eb('attempt_count', '+', 1), next_attempt_at: null, updated_at: new Date() })).where('institution_id', '=', this.institutionId).where('id', '=', id).where('status', 'in', ['PENDING', 'FAILED']).executeTakeFirst()); }
